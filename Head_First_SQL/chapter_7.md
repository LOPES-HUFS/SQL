...
# 추가 : 한 개의 컬럼에 여러개의 값이 들어가 있다면 어떨까? 아마 제대로 다루기 어려울 것이다. 
# 이런 상황을 해결하기 위해, 이번 챕터에서는 단일 테이블에서 여러 개의 테이블로 
# 나누어서 자료를 보관하고 다루는 방법을 배운다. 어떤 기준으로 나누는지, 나누게 되면 어떻게 서로 연결되는지를 잘 살펴보자.  

이번 챕터 '나이젤에게 사랑 찾아주기'미션에서는 `my_contacts`이라는 테이블을 사용한다. 이 테이블은 지금까지 이 교재를 잘 따라왔다면 `gregs_list`이라는 데이터 베이스에 `my_contacts`가  앞에서 만들어져 있을 것이다. 확인해보자.
...
# 추가 : 혹은 기존 테이블이 있더라도 자료 구조가 다를 수 있으니 기존 테이블을 없애도 무방하다
...
## 추가 : 여기서 원자적이지 않다는 말은, 컬럼 1개에 각 행마다 값이 1개가 아니라는 뜻이다.
## 즉, 관심사 열에 1개의 관심사만 있어야 원자적인 데이터가 된다는 뜻이다. 

**To do**: 나이젤을 위한 쿼리 작성해보기

# 주의 : 아래 코드를 돌릴 경우 에러가 나니 참고만 하고 그 밑의 코드를 돌리자.
...
```sql
+------------+-----------+------------+------------------------------+------------+--------------+------------+--------+------------+-------+--------+-----------------------------------------------------------+----------+
| contact_id | last_name | first_name | email                        | birthday   | profession   | phone      | gender | city       | state | status | interests     
                                            | seeking  |
+------------+-----------+------------+------------------------------+------------+--------------+------------+--------+------------+-------+--------+-----------------------------------------------------------+----------+
|          1 | Fiore     | Carla      | cfiore@fioreanimalclinic.com | 1974-01-07 | Veterinarian | 5557894855 | F      | Round Rock | TX    | single | horseback riding, movies, animals, mystery novels, hiking | single M |
+------------+-----------+------------+------------------------------+------------+--------------+------------+--------+------------+-------+--------+-----------------------------------------------------------+----------+
1 row in set (0.020 sec)
```

# 근데 문제는 다른 친구도 소개해달라고 하는 상황입니다. 이런 케이스가 점점 많아지면 코딩하기 번거로워집니다. 
...
(요기에 내용을 추가한다.)
## 7-1 7-2 내용이 여기 아래 어딘가에 들어가면 될 듯하다.


문제가 발생했다. 레기스를 위해 찾아준 사람은 동물도 좋아하지만, 아주 심각한 다른 취미도 있었다. 
동물만 좋아하는 게 아니라 동물로 박제를 하는 취미가 있었던 것이다.
이런 문제를 해결하기 위해선 관심사 열에 대해 더 정확하게 일치하게끔 쿼리를 줘야한다.
만약 `interests`을 쿼리 차원에서 분리하여 `interests1`, `interests2`과 같이 여러 개의 컬럼을 만든다고 해도 
쿼리는 복잡해지고 어려워진다. 또한 SQL에서 지향하는 원자적 데이터에 대한 규칙도 위반하게 된다.

따라서,
초기에 테이블을 잘 설계하면 쿼리의 복잡도가 낮아지며, 원하는 데이터를 더 쉽게 찾을 수 있다. 
그렇다면 테이블을 잘 설계하는 방법은 무엇인가? 그 방법 중 하나가 여러 개의 테이블을 사용하는 것이다. 
여러 개의 테이블을 사용하면 각 카테고리에 대한 세부 정보들을 체계적으로 정리할 수 있다.
그러기 위해선 PRIMARY KEY와 FOREIGN KEY 등의 관계 설정과 기준, 그리고 스키마를 제대로 구성해야한다.
아래부턴 그 내용들에 대해 배울 것이다.
...
위 테이블에서 카테고리 appearance, activities는 여러 개의 데이터가 포함되어 있다. 
만약 해당 테이블에서 여성(F)이고, 큰 발을 가진(huge feet) 사람을 찾는다면 어떻게 쿼리를 줘야 할까?
...
**NOTE**
위 코드에서 사용된 명령어 `SUBSTRING_INDEX(카테고리 이름,'분리단위',위치)`는 한 카테고리에서 지정된 위치까지의 정보를 가져온다. 
즉, 위에서는 카테고리 appearance에서 콤마(,)로 구분된 데이터들 중에 1번째 데이터까지 가져오라는 의미이다.

## 테이블 간의 관계도, 스키마

위에서 말한 체계적인 데이터 관리를 위해선 스키마가 잘 짜져야 한다.
스키마란 데이터베이스 내의 데이터(열들과 테이블들), 그리고 데이터들 사이의 연결 방식에 대한 표현을 뜻한다. 
이 말이 어렵다면, 데이터 테이블 간의 관계가 어떻게 구성되어있는지 알려주는 안내도와 같다고 생각해도 좋다.
...
위의 스키마는 `clown_info`에서 여러 값을 가진 카테고리(컬럼)인 appearance, activities를 
별도의 다른 테이블로 생성해준 뒤 서로 연결한 모습이다. 
지금부터는 어떻게 하나의 테이블을 위 스키마의 형태처럼 어떻게 바꿔주는지 하나씩 살펴볼 것이다.


## 참조키 (foreign key)

각각의 테이블들을 살펴보기에 앞서 위 스키마에서 'foreign key'라고 쓰여있는 카테고리를 찾을 수 있다. 
이는 참조키를 표시한 것인데, 참조키란 무엇이며 왜 사용하는걸까?

쉽게 설명한다면 참조키는 기본키(primary key)와 세트로 사용되며 
각각의 테이블의 레코드 번호를 동일하게 맞추어 테이블 간의 관계를 서로 이어준다. 
즉, 참조키는 다른 테이블과 연결되는 기본키라고도 볼 수 있다. 

참조키의 특징과 규칙은 다음과 같다.

- 참조키는 연결되는 기본키와 다른 이름일 수 있다.
- 참조키에서 참조하는 기본키는 부모키(parent key)라고도 한다. 부모키가 있는 테이블을 부모 테이블이라고 한다.
- 참조키는 한 테이블의 열들이 다른 테이블의 열과 연결되도록 하는데 사용할 수 있다.
- 기본키는 NULL일 수 없지만 참조키는 NULL일 수 있다.
- 참조키는 유일할 필요가 없다. (실제 유일하지 않은 경우도 많다)

참조키에 NULL이 쓰일 필요가 있을까? 고민해보면 생각보다 많은 경우가 있다. 
위에서 사용했던 스키마를 바탕으로 예를 들어보자. 
해당 상황의 조건은 새로 테이블을 만들지 않고 50가지의 activity 항목이 있는 기존 테이블을 사용해야 한다는 전제를 가진다.

Situation: 50가지의 activity 항목이 있는 테이블이 있으며, 우리는 `clown_info` 테이블의 키본키를 사용하여 
각 광대들의 activity 들을 연결하려고 한다.

위 상황에서 50가지 항목 중에는 광대들의 activity가 속하는 항목도 있고 아닌 항목도 있다. 
이런 경우 해당하지 않는 항목의 참조키에는 NULL이 포함된다.

쉽게 표현해서, 액티비티이든 관심사 컬럼이든, 사람마다 관심사와 액티비티의 갯수가 다를 것인데, 이렇게 되면
적은 관심사나 액티비티 갯수를 가진 사람의 컬럼 중 일부는 그 갯수만큼 NULL 값이 들어갈 수밖에 없기 때문이다.

## 참조키의 제약조건

하지만 만약 위와 같은 경우가 아니라, 부모 테이블의 기본키를 참조키로 그대로 0연결하려면 어떻게 해야할까? 
즉, 해당 자식 테이블은 NULL값이 없고, 참조키는 무조건 부모 테이블의 기본키 값 중 하나를 가져야 되는 상황일 때 말이다. 
이런 제약조건을 '참조 무결성(referential integrity)'라고 한다.
즉, 참조키와 기본키가 서로 일치하게끔 구성되는 케이스인 것이다.

제약조건을 포함하여 참조키를 설정하는 방법은 아래와 같다. 
아래의 코드는 위 스키마에서 `activities` 테이블을 만드는 코드이다. 
해당 참조키는 부모 테이블인 `clown_info`의 기본키를 제약조건으로 설정한다.
...
위 코드는 id 카테고리를 생성한 후 참조키로 지정한다. 
명령어 `CONSTRAINT`는 해당 참조키가 어떤 테이블의 기본키와 제약조건으로 묶이는지를 정의한 것이며, 
명령어 `FOREIGN KEY`는 해당 카테고리가 참조키임을 나타낸 것이다. 
마지막으로 `REFERENCES`는 부모 테이블과 기본키 정보를 저장한다.
...
그렇다면 왜 굳이 참조키를 설정해야 하는가? 굳이 참조키를 설정하지 않고 테이블을 따로 관리해도 괜찮지 않을까? 
이에 대한 답변으로는 고아(orphan)를 만들지 않기 위해서이다.
쉽게 설명하면, 참조키로 설정을 해놓아야, 참조키와 연결된 테이블의 기본키를 실수로 삭제할 수 없게끔 설정된다.

만약 수많은 자식 테이블에 참조키로 들어가 있는 부모 테이블의 레코드가 삭제된다면 어떻게 될까? 
부모를 잃은 자식 테이블들의 레코드는 쓸모없는 존재가 된다. 
즉, 불러올 대상이 없는 숫자가 되는 셈이다. 이 부분을 막기 위해서 참조키의 제약조건을 설정한다. 
이렇게 제약조건을 설정하면 부모 테이블의 기본키가 참조된 자식 테이블을을 삭제해야 부모 테이블의 정보를 삭제할 수 있다. 
이해가 어렵다면 아래의 예시를 살펴보자.
...
따라서 제약조건이 설정돼 있는 것이 오히려 효율적인 테이블 관리를 가능케 한다.

## 테이블간의 관계

지금까지는 테이블들을 연결하는 방법에 대해서 알아보았다. 
그렇다면 테이블들을 서로 어떻게 연결하는 것이 효율적일까? 

이제부터는 데이블간의 관계 종류를 살펴보자.
...
글로만 이해하기는 어려울 수 있다. 
따라서 예시로 우리가 만들 테이터베이스의 스키마를 다시 한 번 상기 시켜보자.
...
일대다 관계는 부모 테이블 기본키 하나가 자식 테이블의 참조키로 여러번 들어가 있는 경우이다. 
예를 들면 위에서 살펴봤던 부모 테이블인 `clown_info`와 자식 테이블인 `activities`가 여기에 해당한다. 
`clown_info`에서 기본키인 `2`가 자식 테이블인 `activities`에 여러 번 들어가 있는 것을 확인할 수 있다.
